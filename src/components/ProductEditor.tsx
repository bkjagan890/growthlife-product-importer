...
import { loadProduct, saveDrafts, summaryFor, uploadImage } from "../lib/actions";
...
export function ProductEditor({ productId, onClose, onSaved }: Props) {
  ...
  async function save() {
    ...
    try {
      ...
      const [r] = await saveDrafts([payload], false);
      if (!r.ok) {
      ...
      setDirty(false);
      onSaved(r.id ? await summaryFor(r.id).catch(() => null) : null);
    } catch (e: any) {
      setError(e.message);
    } finally {
      setSaving(false);
