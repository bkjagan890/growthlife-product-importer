...
// Minimal shape of the admin client from @shopify/shopify-app-react-router
...
export async function gql<T = any>(admin: AdminClient, query: string, variables?: Record<string, unknown>): Promise<T> {
  ...
  for (let attempt = 0; attempt < 4; attempt++) {
    try {
      ...
      lastErr = e;
      // the admin client throws GraphqlQueryError for errors too
      const msg = String(e?.message ?? e);
      if (/throttled/i.test(msg) && attempt < 3) {
        await sleep(1000 * (attempt + 1));
        continue;
      }
      if (e?.body?.errors?.graphQLErrors?.length) {
        throw new ShopifyError(e.body.errors.graphQLErrors.map((x: any) => x.message).join("; "));
      }
      throw e;
    }
  }
...
export async function stagedUpload(admin: AdminClient, file: { buffer: Buffer | Uint8Array; filename: string; mimeType: string }) {
  const data = await gql(admin, `#graphql
  ...
  form.append("file", new Blob([file.buffer as any], { type: file.mimeType }), file.filename);
