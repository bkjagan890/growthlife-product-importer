// Excel (.xlsx) and CSV reading/writing for GrowthLife Product Importer.
...
export interface ExportOptions {
  ...
  thumbnails?: (Buffer | null | undefined)[];
  template?: boolean;
}

export async function buildWorkbook(opts: ExportOptions): Promise<Buffer> {
...
export async function readWorkbook(buffer: Buffer): Promise<ReadResult> {
  const wb = new ExcelJS.Workbook();
...
async function readInCellImages(buffer: Buffer, sheetName: string, wb: ExcelJS.Workbook): Promise<EmbeddedImage[]> {
  ...
  for (const m of sheetXml.matchAll(/<c\b([^>]*)\bvm="(\d+)"([^>]*)>/g)) {
    ...
    out.push({
      row,
      col,
      buffer: Buffer.from(await file.async("uint8array")),
