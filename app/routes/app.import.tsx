...
import { downloadFile, fetchJson, postJson, toast } from "../lib/client";
...
export default function ImportPage() {
  ...
  async function parse(f: File) {
    ...
    try {
      ...
      fd.append("file", f);
      const data = await fetchJson("/app/api/parse", { method: "POST", body: fd });
      setParsed(data);
    } catch (e: any) {
      setError(e.message);
    } finally {
  ...
  async function run() {
    ...
    for (let i = 0; i < parsed.drafts.length; i += BATCH) {
      ...
      try {
        const res = await postJson<{ results: Result[] }>("/app/api/product", { drafts: batch, keepWhenEmpty: true });
  ...
  return (
    <div className="gl-page">
      <div className="gl-head">
        ...
        <button className="gl-btn" onClick={() => downloadFile("/app/api/export?template=1", "growthlife-import-template.xlsx").catch((e) => toast(e.message, true))}>
          ⬇ Blank template
        </button>
        <Link className="gl-btn" to="/app/export">⬇ Export current products</Link>
      </div>
      ...
      <div className="gl-card">
        ...
        >
          {parsing ? (
            <><span className="gl-spinner" /> Reading {file?.name} and uploading pasted pictures…</>
          ) : (
            <>
      ...
      {parsed && (
        <div className="gl-card">
          ...
          <div style={{ display: "flex", gap: 8 }}>
            ...
            {results.length > 0 && !running && <Link className="gl-btn" to="/app">View products</Link>}
          </div>
        </div>
      )}
    </div>
